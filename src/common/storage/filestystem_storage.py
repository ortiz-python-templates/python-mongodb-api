...
class FileSystemStorage(BaseStorage):
    ...
    def __init__(self):
        ...
        self.provider_name = StorageProvider.FILESYSTEM
    ...
    async def upload(self, file: UploadFile, storage_path: str) -> UploadInfo:
        ...
        size = await self._save_uploaded_file(file, storage_path, file_name)

        object_key = f"{storage_path}/{file_name}"
        metadata = {}
        ...
        return UploadInfo(
            ...
            bucket_name=self.destination_path,
            metadata=metadata
        )


    async def upload_multiple_files(self, files: List[UploadFile], storage_path: str) -> List[UploadInfo]:
    ...
    async def download(self, object_key: str) -> BytesIO:
        full_path = os.path.join(self.destination_path, object_key)
        if not os.path.exists(full_path):
            raise FileNotFoundError(f"File '{object_key}' not found on disk")
    ...
    async def _save_uploaded_file(self, file: UploadFile, storage_path: str, file_name: str) -> int:
